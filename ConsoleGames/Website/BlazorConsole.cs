        ...
        get
        {
            ...
            for (int row = 0; row < View.GetLength(0); row++)
            {
                for (int column = 0; column < View.GetLength(1); column++)
                {
                    ...
                    {
                        bool isDark =
                            (View[row, column].Char is '█' &&
                             View[row, column].ForegroundColor is ConsoleColor.White) ||
                    ...
                    {
                        stateBuilder.Append(
                            $@"<span style=""background-color:{HtmlEncode(View[row, column].BackgroundColor)}"">");
                    }
    ...
    public async Task PromptPressToContinue(string? prompt = null, ConsoleKey key = ConsoleKey.Enter)
    {
        if (!key.IsDefined())
        {
            throw new ArgumentOutOfRangeException(nameof(key), key,
                $"{nameof(key)} is not a defined value in the {nameof(ConsoleKey)} enum");
        }
